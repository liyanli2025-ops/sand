...
function _runFinalePreludeAndIgnite(){
  ...
  if(!el){
    ...
    el.innerHTML = ''
      + '<div class="pl-line pl-line-1">五片碎光，已在你手中。</div>'
  ...
  // 3.5s 容器整体淡出
  _T(() => { el.classList.add('fade-out'); }, 3500);
  // 4.3s 文字几乎消失的瞬间 = 爆炸
  _T(() => {
    if(mdlG._collapsePaused) resumeCollapseFromDirectory();
  }, 4300);
  // 4.6s DOM 清理（爆炸已启动 0.3s，前景的余韵也已淡到几乎不可见）
  _T(() => {
...
function setupDirectoryFragments(){
  ...
  if(!_dirCueEl){
    ...
    _dirCueEl.innerHTML = '轻触<strong class="dir-cue-em">任意 5 个碎片</strong>，让一些光，在黯淡之前再被看见一次';
...
function ensureDirectoryDom(){
  if(!_dirHintEl){
    ...
    for(let i = 0; i < slots.length; i++){
      ...
      slotsHtml += '<button class="deck-slot" data-idx="' + i + '" disabled aria-label="未拾起的碎片">'
    ...
    _dirHintEl.querySelectorAll('.deck-slot').forEach((btn) => {
      btn.addEventListener('click', () => {
        ...
        const idx = parseInt(sceneAttr, 10);
        if(isNaN(idx)) return;
        // 已是当前场景就忽略
